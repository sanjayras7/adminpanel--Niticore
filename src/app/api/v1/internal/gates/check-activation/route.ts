...
import { canActivateTenant } from '@/lib/gate-service'
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    const result = await canActivateTenant(body.organization_id, body.override_id || undefined)
    return NextResponse.json({ data: result })
  } catch (err) {
    console.error('[GATES] check-activation error:', err)
    return NextResponse.json({ error: 'internal_error', message: 'Failed to check activation gate' }, { status: 500 })
  }
}
