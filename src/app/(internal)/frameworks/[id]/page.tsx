...
export default function FrameworkDetailPage() {
  ...
  if (loading) return <p style={{ color: '#666' }}>Loading framework...</p>

  if (error) {
    return (
      <div style={{ padding: 12, background: '#fff0f0', border: '1px solid #fcc', borderRadius: 4 }}>
        <p style={{ margin: 0, color: '#c00', fontSize: 14 }}>{error}</p>
        <Link href="/frameworks" style={{ color: '#1a1a2e', fontSize: 14 }}>Back to frameworks</Link>
      </div>
    )
  }
  ...
  return (
    <div>
      <Link href="/frameworks" style={{ color: '#666', fontSize: 13, textDecoration: 'none', display: 'inline-block', marginBottom: 12 }}>&larr; Back to frameworks</Link>

      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 24, margin: '0 0 4px' }}>{framework.name}</h1>
          <p style={{ margin: 0, color: '#666', fontSize: 14 }}>{framework.description || 'No description'}</p>
          <p style={{ margin: '4px 0 0', color: '#999', fontSize: 12 }}>{framework.version_count} version(s)</p>
        </div>
      </div>
      ...
      {framework.versions.length === 0 && (
        <div style={{ padding: 40, textAlign: 'center', color: '#888', background: '#fff', borderRadius: 8, border: '1px solid #eee' }}>
          ...
          {canMutate && <p style={{ fontSize: 13, margin: 0 }}>Create the first version to add sections and clauses.</p>}
        </div>
      )}
            ...
            {framework.versions.map((v) => (
              <tr key={v.id} style={{ borderBottom: '1px solid #f0f0f0' }}>
                <td style={{ padding: '10px 16px' }}>
                  <Link href={`/frameworks/${id}/versions/${v.id}`} style={{ color: '#1a1a2e', textDecoration: 'none', fontWeight: 500 }}>
                    {v.version_label}
                  </Link>
                </td>
                <td style={{ padding: '10px 16px' }}>
                ...
                <td style={{ padding: '10px 16px', textAlign: 'right', fontSize: 13 }}>
                  <Link href={`/frameworks/${id}/versions/${v.id}`} style={{ color: '#1a1a2e', textDecoration: 'none', marginRight: canMutate ? 8 : 0 }}>
                    View
                  </Link>
